...
function audit(action, details = {}, req = null) {
  ...
  securityAudit(safeAction, safeDetails);

  let actorType = "system";
  let actorId = null;
  let actorLabel = null;

  if (safeAction.startsWith("admin_")) {
    actorType = "admin";
    actorLabel = process.env.ADMIN_USERNAME || "admin";
  } else if (
    safeDetails.user_id !== undefined &&
    safeDetails.user_id !== null
  ) {
    const parsedUserId = Number(safeDetails.user_id);

    actorType = "student";
    actorId = Number.isInteger(parsedUserId)
      ? parsedUserId
      : null;
  }
...
async function issueAuthToken(userId, purpose, expiresInMinutes) {
  ...
  async (req, res) => {
    try {
      ...
      const insertResult = await pool.query(`
        INSERT INTO users
        (
          name,email,password,department,campus,
          student_id,research_interest,email_verified
        )
      ...
      const verificationToken =
        await issueAuthToken(newUser.id, "verify_email", 24 * 60);
      ...
      res.json({
        ...
        message: emailSent
          ...
          : "Registration saved, but the verification email could not be sent. Try resend verification later."
      });
    } catch (error) {
      if (error.code === "23505") {
...
app.post(
  ...
  async (req, res) => {
    const genericResponse = {
      success: true,
      message: "If the account exists and still needs verification, a new email will be sent."
    };
    ...
    try {
      ...
      const rawToken =
        await issueAuthToken(user.id, "verify_email", 24 * 60);
      ...
      try {
        ...
        audit("verification_email_resent", {
          user_id: user.id
        });
      } catch (mailError) {
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      res.json({
        success: true,
        message: "Email verified successfully. You can log in after admin approval."
      });
    } catch (error) {
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const rawToken =
        await issueAuthToken(user.id, "reset_password", 30);
...
app.get("/api/admin/stats", requireAdmin, async (req, res) => {
  try {
    const result = await pool.query(`SELECT (SELECT COUNT(*)::int FROM users WHERE role='student') AS students, (SELECT COUNT(*)::int FROM users WHERE role='student' AND status='Pending') AS applications, (SELECT COUNT(*)::int FROM documents) AS documents, (SELECT COUNT(*)::int FROM events) AS events`);
    res.json(result.rows[0]);
  } catch (error) { console.error("Admin stats error:", error); res.status(500).json({error:"Something went wrong"}); }
});
...
app.post("/api/admin/events", requireAdmin, async (req, res) => {
  try {
    ...
    audit("admin_event_created", { event_id: result.rows[0].id }, req);
...
app.get("/api/admin/audit-logs", requireAdmin, async (req, res) => {
  try {
    ...
    const actorType = ["admin", "student", "system"].includes(
      String(req.query.actor_type || "").trim()
    )
      ? String(req.query.actor_type).trim()
      : "";
    ...
    res.json({
      ...
      total_pages: Math.max(
        1,
        Math.ceil(totalResult.rows[0].total / limit)
      )
    });
  } catch (error) {
