...
app.patch("/api/profile",requireStudent,async(req,res)=>{
  try{
    const {
      name,department,campus,student_id,research_interest,bio
    }=req.body;
...
app.get("/api/admin/notifications",requireAdmin,async(req,res)=>{
  try{
    const result=await pool.query(`
      SELECT
        n.*,
        u.name AS target_student_name
      FROM notifications n
      LEFT JOIN users u
      ON u.id=n.target_student_id
      ORDER BY n.created_at DESC, n.id DESC
    `);
...
app.post("/api/admin/notifications",requireAdmin,async(req,res)=>{
  try{
    ...
    const allowedAudiences=["all","campus","student"];

    if(!allowedAudiences.includes(audience||"all")){
